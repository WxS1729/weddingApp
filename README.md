...
我的婚礼邀请函小程序，可以搜「李的王冠婚礼邀请函」或扫描下方二维码来查看。

![输入图片说明](gh_c38b7290e296_258.jpg)
...
- 接着是「云函数」，我写了两个云函数 `surveys` 和 `greetings`，分别做报名和读取祝福语用的，关键是要修改「云函数权限」，修改自定义安全规则，改成「允许所有用户访问」，不然分享到朋友圈后，从朋友圈点击进去会进入「单页模式」，这个模式下它只是个快照，并没有登录态，就会导致拉取祝福语时因为没有权限而拉取失败（拉取的祝福语会在左下角做动画展示）；
